...
/**
 * This behavior overrides native scrolling for an area. This area can be a single defined part of a page, the entire
 * page, or several different parts of a page.
 * 
 * To use this scrolling behavior you need to define a frame and the content. The frame defines the area that the
 * content will scroll within. The frame and content must both be HTML Elements, with the content being a direct child
 * of the frame. Usually the frame is smaller in size than the content. This is not necessary though, if the content is
 * smaller then bouncing will occur to provide feedback that you are past the scrollable area.
 * 
 * <?> The scrolling behavior works using the webkit translate3d transformation, which means browsers that do not have
 * hardware accelerated transformations will not perform as well using this. Simple scrolling should be fine even
 * without hardware acceleration, but animating momentum and deceleration is unacceptably slow without it.
 * 
 * For this to work properly you need to set -webkit-text-size-adjust to 'none' on an ancestor element of the frame, or
 * on the frame itself. If you forget this you may see the text content of the scrollable area changing size as it
 * moves.
 * 
 * Browsers that support hardware accelerated transformations: - Mobile Safari 3.x </?>
 * 
 * The behavior is intended to support vertical and horizontal scrolling, and scrolling with momentum when a touch
 * gesture flicks with enough velocity.
 */
public class Scroller implements Momentum.Delegate, TouchHandler.DragDelegate, TouchHandler.TouchDelegate {
    ...
    /**
     * The offset of the scrollable content when a touch begins. Used to track delta x and y's of the scrolling content.
     */
    private Point contentStartOffset;

    /**
     * Frame is the node that will serve as the container for the scrolling content.
     */
    ...
    /**
     * The minimum coordinate that the left upper corner of the content can scroll to.
     */
    ...
    /**
     * The start position of a touch. Used to track delta x and y's of the scrollable content.
     */
    private Point touchStartPosition;

    @SuppressWarnings("unused")
	private MobileScrollPanelInternal container;

    private List<ScrollHandler> myScrollHandlers = new ArrayList<ScrollHandler>();

    /**
     * Creates a new scroller
     * 
     * The frame needs to have its dimensions set, and the scrollable content will be allowed to move within those
     * dimensions. It is required that the layer element be a direct child node of the frame.
     * 
     * @param theContainer
     * 
     * @param frame
     ...
     * @param layer
     *            the element that is the scrolling content
     */
    public Scroller(MobileScrollPanelInternal theContainer, Element frame, Element layer) {
    ...
    /**
     * Provide access to the touch handler that the scroller created to manage touch events.
     * 
    ...
    /**
     * Recalculate dimensions of the frame and the content. Adjust the minPoint allowed for scrolling. Call this method
     * if you know the frame or content has been updated. Called internally on every touchstart event the frame
     * receives.
     */
    public void reconfigure() {
        scrollSize = new Point(frame.getOffsetWidth(), frame.getOffsetHeight());
        contentSize = new Point(layer.getScrollWidth(), layer.getScrollHeight());

        Point adjusted = getAdjustedContentSize();
        minPoint = new Point(scrollSize.x - adjusted.x, scrollSize.y - adjusted.y);
    }
    ...
    /**
     * Adjust the new calculated scroll position based on the minimum allowed position.
     * 
     * @param y
     *            The new position before adjusting.
     * @param y2
    ...
    /**
     * Adjusted content size is a size with the combined largest height and width of both the content and the frame.
     * 
     * @return the adjusted size.
     */
    private Point getAdjustedContentSize() {
        return new Point(Math.max(scrollSize.x, contentSize.x), Math.max(scrollSize.y, contentSize.y));
    }

    /**
     * Initialize the dom elements necessary for the scrolling to work. - Sets the overflow of the frame to hidden.
     * 
     * - Asserts that the content is a direct child of the frame.
     */
    private void initLayer() {
        assert layer.getParentNode() == frame : "The scrollable node provided to Scroller must be " + "a direct child of the scrollable frame.";
    ...
    /**
     * Whether or not the scrollable area should scroll horizontally or not. Only returns true if the client has enabled
     * horizontal scrolling, and the content is wider than the frame.
     * 
    ...
    /**
     * In the event that the content is currently beyond the bounds of the frame, snap it back in to place.
     */
    private void snapContentOffsetToBounds() {
        Point point = new Point(clamp(minPoint.x, contentOffset.x, 0), clamp(minPoint.y, contentOffset.y, 0));
    ...
    private boolean startDeceleration(Point velocity) {
        ...
        return momentum.start(velocity, minPoint, ORIGIN, contentOffset);
    }

    public void addScrollHandler(ScrollHandler theHandler) {
        myScrollHandlers.add(theHandler);
    }
}